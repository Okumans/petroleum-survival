...
Texture::Texture(const std::vector<std::string> &faces)
  ...
  bool is_hdr = false;
  if (!faces.empty()) {
    std::string_view first_face = faces[0];
    if (first_face.ends_with(".hdr")) {
      is_hdr = true;
    }
  }
  ...
  for (unsigned int i = 0; i < faces.size(); i++) {
    void *data = nullptr;
    if (is_hdr) {
      data = stbi_loadf(faces[i].c_str(), &width, &height, &nr_channels, 0);
    } else {
      // Force 4 channels (RGBA) to ensure consistency for LDR
      data = stbi_load(faces[i].c_str(), &width, &height, &nr_channels, 4);
    }
...
GLuint Texture::_loadTexture(const char *path, bool flip_vertical) {
  ...
  GLuint texture_id = _createTexture(data, width, height, nr_components);
  stbi_image_free(data);
  return texture_id;
}
...
GLuint Texture::_loadTextureFromMemory(const void *data, size_t size,
  ...
  GLuint texture_id = _createTexture(pixels, width, height, nr_components);
...
GLuint Texture::_createTexture(unsigned char *data, int width, int height,
  ...
  glTextureStorage2D(texture_id, levels, internal_format, width, height);

  // 5. Handle Alignment for non-power-of-two/odd dimensions (418px, 350px)
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

  // 6. Upload Data (DSA)
  glTextureSubImage2D(texture_id, 0, 0, 0, width, height, data_format,
                      GL_UNSIGNED_BYTE, data);
