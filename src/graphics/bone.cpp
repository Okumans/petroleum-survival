...
void Bone::update(float animation_time) {
  glm::vec3 pos = _interpolatePosition(animation_time);
  glm::quat rot = _interpolateRotation(animation_time);
  glm::vec3 scale = _interpolateScaling(animation_time);

  m_localTransform = glm::translate(glm::mat4(1.0f), pos);
...
uint32_t Bone::getPositionIndex(float animation_time) const {
  if (animation_time < m_positions[m_lastPositionIndex].timeStamp) {
    m_lastPositionIndex = 0;
  }

  for (size_t index = m_lastPositionIndex; index < m_numPositions - 1;
       ++index) {
...
uint32_t Bone::getRotationIndex(float animation_time) const {
  if (animation_time < m_rotations[m_lastRotationIndex].timeStamp) {
    m_lastRotationIndex = 0;
  }

  for (size_t index = m_lastRotationIndex; index < m_numRotations - 1;
       ++index) {
    if (animation_time < m_rotations[index + 1].timeStamp) {
      m_lastRotationIndex = index; // Update cache
      return index;
    }
  }
...
uint32_t Bone::getScaleIndex(float animation_time) const {
  if (animation_time < m_scales[m_lastScaleIndex].timeStamp) {
    m_lastScaleIndex = 0;
  }

  for (size_t index = m_lastScaleIndex; index < m_numScalings - 1; ++index) {
    if (animation_time < m_scales[index + 1].timeStamp) {
      m_lastScaleIndex = index; // Update cache
      return index;
    }
  }
...
glm::vec3 Bone::_interpolatePosition(float animation_time) const {
  if (1 == m_numPositions) {
  ...
  return glm::mix(m_positions[p0_index].position,
                  m_positions[p1_index].position, scale_factor);
}

glm::quat Bone::_interpolateRotation(float animation_time) const {
  if (1 == m_numRotations) {
...
glm::vec3 Bone::_interpolateScaling(float animation_time) const {
  if (1 == m_numScalings) {
  ...
  return glm::mix(m_scales[p0_index].scale, m_scales[p1_index].scale,
                  scale_factor);
}
...
//       glm::mix(m_positions[p0_index].position,
//       m_positions[p1_index].position,
//                scale_factor);
