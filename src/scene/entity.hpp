...
public:
    ...
    void setKnockbackResist(float resist) { m_knockbackResist = std::clamp(resist, 0.0f, 1.0f); }

    virtual void takeDamage(float amount, bool isCritical, glm::vec3 knockbackDir = glm::vec3(0.0f), float knockbackForce = 0.0f) {
        if (m_isDead || m_removeRequested) return;
    ...
    virtual void onDeath() {
        m_isDead = true;
        // Basic default behavior, the Game loop should listen for this and handle drops
        requestRemoval(); 
    }
};
