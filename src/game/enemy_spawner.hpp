...
private:
  ...
  float m_onScreenRadius =
      12.0f; // Spawns within this radius are considered on-screen
  float m_offScreenRadius =
      25.0f; // Spawns beyond on-screen radius but within this
...
public:
  ...
  void spawnEnemy(glm::vec3 position, float health_multiplier);
  void spawnSpecificEnemy(GameObjectType type, glm::vec3 pos,
                          float health_multiplier = 1.0f, int tier = 0);
  void spawnInCircle(int count, float radius, float health_multiplier);

  // New: spawn with mixed on-screen and off-screen distribution
  void spawnMixed(int count, float health_multiplier);

  // Setters for spawn config
  void setOnScreenSpawnFraction(float fraction) {
    m_onScreenSpawnFraction = fraction;
  }
