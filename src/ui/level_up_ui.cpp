...
LevelUpUI::LevelUpUI(UIManager &ui_manager)
...
void LevelUpUI::applySelection() {
  if (m_selectedIndex >= 0 && m_selectedIndex < static_cast<int>(m_upgrades.size()) && m_game) {
...
void LevelUpUI::_createUpgradeButtons() {
  ...
  for (size_t i = 0; i < m_upgrades.size(); ++i) {
    ...
    m_uiManager.addInteractiveElement(
        buttonName, bounds, glm::vec4(0.2f, 0.2f, 0.8f, 0.8f),
        [this, index]() {
